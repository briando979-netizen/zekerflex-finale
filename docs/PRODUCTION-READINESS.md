...
| ✅ grotendeels | **Accessibility (WCAG 2.1 AA)** | `npm run test:a11y` — `axe-core` (`@axe-core/playwright`) over de 7 publieke pagina's + de 3 rol-dashboards, gate = **0 serious/critical-overtredingen, contrast inbegrepen** (`e2e/a11y.spec.ts`); groen op alle 10. Gefixt: unlabeled inputs op `/register` (critical), redundante image-link op `ShiftCard`, radiogroep-role, en een **contrast-pass**: `neutralx.400` `#8A93A0`→`#6B7280`, donkere-band-tekst `text-white/35…45`→`/60–65`, admin `--a-mute`→`#5F6774`, nieuw `--a-accent-ink` `#0B6E54` voor groene micro-labels, uitzend-CTA nu donkere tekst op het felle oranje (7:1). `docs/ACCESSIBILITY.md` = criteria + wat er veranderd is + de handmatige checklist (toetsenbord, screenreader, zoom/reflow, `prefers-reduced-motion`). axe dekt ~⅓ van WCAG. **Nog te doen:** één externe specialist-audit; captions op de uitleg-video's; de handmatige checklist één keer echt aflopen. |
| ✅ gedaan | **KYC-onboarding: geen dubbele ID-upload meer + beeldcontrole gebouwd** | `/dashboard/verificatie` vroeg twee keer om hetzelfde identiteitsbewijs (het zelf-verificatieformulier én, los daaronder, `ComplianceDocsPanel`). `OnboardingForm` is nu 3 losse camera-opnames (voorkant/achterkant/selfie, `capture="environment"`/`"user"`); `submitFreelancerOnboarding` (`lib/onboarding/verify.ts`) vult de "Identiteitsbewijs"-slot automatisch uit de voorkant-foto (niet bij een rijbewijs of een afwijzing — die twee gevallen behouden hun eigen upload-eis). Daarnaast **`lib/kyc/vision.ts`**: een echte beeldcontrole (documentplausibiliteit + gezichtsvergelijking + spoof-heuristiek) via een lokaal vision-model over Ollama — de tekst-checker keek nooit naar de foto-inhoud. **Bewust uit (`LLM_VISION_MODEL` leeg by default)**: getest op CPU-only hardware met `moondream` — 45-150s per call en onbetrouwbare JSON-output; niet productie-waardig zonder GPU + een sterker model, of een los face-embedding-model i.p.v. een algemene VLM voor de gezichtsvergelijking. Code + gedegradeerd gedrag (nooit blokkerend, valt terug op "in review") is wel volledig gebouwd en getest. Tests: `tests/onboarding-verify.test.ts` (6), `tests/kyc-vision.test.ts` (7). |
